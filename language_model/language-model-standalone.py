...
def build_opt(
    ...
    """
    Load OPT model/tokenizer but shard automatically across all available GPUs.
    Tries 8-bit quant (bitsandbytes) first; if not available falls back to
    device_map="auto" with CPU offload.
    """

    # ensure offload folder exists (for CPU/disk offload fallback)
    offload_folder = os.path.expanduser(os.path.join(cache_dir or ".", "opt_offload"))
    ...
    # Build max_memory dict for device_map
    # We allocate a conservative per-GPU budget to leave headroom for activations.
    max_memory = {}
    try:
    ...
    if n_gpus > 0:
        # reserve ~10 GiB per RTX 2080 Ti; adjust smaller if needed
        per_gpu_mem = "10GiB"
        for i in range(n_gpus):
            max_memory[f"cuda:{i}"] = per_gpu_mem
        # set a large CPU fallback
        max_memory["cpu"] = "200GiB"
    else:
        # CPU-only machine
        max_memory["cpu"] = "200GiB"

    # Try load_in_8bit first (if bitsandbytes is installed)
    try:
        print("Attempting 8-bit load_in_8bit=True (bitsandbytes)...")
        model = AutoModelForCausalLM.from_pretrained(
            model_name,
            cache_dir=cache_dir,
            load_in_8bit=True,
        ...
        model.eval()
        print("Loaded model in 8-bit across devices.")
        return model, tokenizer
    except Exception as e:
        print("8-bit load not available or failed:", e)
        print("Falling back to device_map='auto' with CPU offload (may be slower).")

    # Fallback: device_map="auto" with CPU offload and fp16 where possible
    try:
        model = AutoModelForCausalLM.from_pretrained(
            model_name,
            cache_dir=cache_dir,
        ...
        model.eval()
        print("Loaded model with device_map='auto' and CPU offload.")
        return model, tokenizer
    except Exception as e:
        print("device_map auto + offload failed:", e)
        # last resort: load on CPU (very slow)
        print("Attempting CPU load as last resort (very slow).")
        model = AutoModelForCausalLM.from_pretrained(
            model_name,
            cache_dir=cache_dir,
            device_map={"": "cpu"},
