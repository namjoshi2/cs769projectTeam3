...
from bi_rnn_model import BiGRUDecoder
...
class BrainToTextDecoder_Trainer:
    ...
    def __init__(self, args):
        ...
        elif self.args['lr_scheduler_type'] == 'step': # *** ADD STEP DECAY ***
            self.learning_rate_scheduler = self.create_step_lr_scheduler(self.optimizer)
    ...
    def create_optimizer(self):
        ...
        # *** Changed from AdamW to SGD ***
        optim = torch.optim.SGD(
            param_groups,
            lr = self.args['lr_max'],
            momentum = self.args['momentum'], # New argument
            nesterov = self.args['nesterov'], # New argument
            weight_decay = self.args['weight_decay'],
        )

        return optim

    def create_step_lr_scheduler(self, optim):
        '''
        Create lr scheduler that implements step decay
        '''
        step_size = self.args['step_size']
        gamma = self.args['gamma']
        step_size_day = self.args['step_size_day']
        gamma_day = self.args['gamma_day']

        def step_lambda(current_step, step_size, gamma):
            '''
            Calculates the decay multiplier based on the number of steps
            '''
            # Calculate the number of times the LR has been stepped down
            num_steps = current_step // step_size 
            return gamma ** num_steps

        if len(optim.param_groups) == 3:
            lr_lambdas = [
                lambda step: step_lambda(step, step_size, gamma), # biases 
                lambda step: step_lambda(step, step_size_day, gamma_day), # day params
                lambda step: step_lambda(step, step_size, gamma), # rest of model weights
            ]
    ...
    def train(self):
        ...
        # set model to train and move to device (device must be set earlier)
        self.model.to(self.device)
        self.model.train()

        # scaler = torch.cuda.amp.GradScaler(enabled=self.args.get('use_amp', True))
        ...
        for i, batch in enumerate(self.train_loader):
            print(f"Loaded training batch {i}", flush=True)
            ...
            # Move data to device (ensure all relevant tensors moved)
            ...
            # Autocast for AMP (use torch.cuda.amp.autocast)
            # with torch.cuda.amp.autocast(enabled=self.args.get('use_amp', True), dtype=torch.bfloat16):
            with torch.autocast(device_type = "cuda", enabled = self.args['use_amp'], dtype = torch.bfloat16):
                ...
                adjusted_lens = adjusted_lens.to(self.device)

                # forward
                logits = self.model(features, day_indicies)

                # ctc expects (T, N, C) so permute accordingly and compute log_softmax
            ...
            if i % self.args['batches_per_train_log'] == 0:
                self.logger.info(f'Train batch {i}: loss: {(loss.detach().item()):.2f} grad norm: {grad_norm:.2f} time: {train_step_duration:.3f}')

            # Free large tensors immediately
            del logits, log_probs, loss, features, labels, n_time_steps, phone_seq_lens, day_indicies, adjusted_lens
            torch.cuda.empty_cache()

            # validation block (unchanged, but ensure validation moves tensors to device inside validation)
        ...
        if self.args['save_final_model'] and len(val_PERs) > 0:
    ...
    def validation(self, loader, return_logits = False, return_data = False):
        ...
        for i, batch in enumerate(loader):
            print(f"[Heartbeat] Loaded validation batch {i}", flush=True)          
