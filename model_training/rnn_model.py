...
class BidirectionalGRUDecoder(nn.Module):
    """
    Bidirectional GRU-based decoder adapted from the original GRUDecoder.
    """
    ...
    def __init__(self,
        ...
        self.gru = nn.GRU(
            ...
            hidden_size=self.n_units,
            num_layers=self.n_layers,
            dropout=self.rnn_dropout if n_layers > 1 else 0.0,
            batch_first=True,
            bidirectional=True
        )
        ...
        # --- 🔁 Adjusted output layer (hidden * 2 for both directions) ---
        self.out = nn.Linear(self.n_units * 2, self.n_classes)
        nn.init.xavier_uniform_(self.out.weight)

        # Learnable initial hidden states (2 directions * n_layers)
    ...
    def forward(self, x, day_idx, states=None, return_state=False):
        ...
        if states is None:
            states = self.h0.expand(-1, x.shape[0], -1).contiguous()
