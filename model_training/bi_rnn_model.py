...
class BiGRUDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        # Prediciton head. Weight init to xavier
        # Bidirectional GRU output size is 2 * hidden_size
        self.out = nn.Linear(self.n_units * 2, self.n_classes) # **UPDATED INPUT SIZE**
        nn.init.xavier_uniform_(self.out.weight)

        # # Learnable initial hidden states
        # self.h0 = nn.Parameter(nn.init.xavier_uniform_(torch.zeros(1, 1, self.n_units)))

        # Learnable initial hidden states: n_layers * 2 (for bidirectional)
        self.h0 = nn.Parameter(nn.init.xavier_uniform_(torch.zeros(self.n_layers * 2, 1, self.n_units))) # **UPDATED DIMENSION**
    ...
    def forward(self, x, day_idx, states = None, return_state = False):
        ...
        # Determine initial hidden states. For BiGRU, first dim is n_layers * 2
        if states is None:
            states = self.h0.expand(self.n_layers * 2, x.shape[0], self.n_units).contiguous() # **UPDATED DIMENSION**
