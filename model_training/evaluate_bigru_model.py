...
from rnn_model import BidirectionalGRUDecoder
from evaluate_model_helpers import *

# argument parser for command line arguments
parser = argparse.ArgumentParser(description='Evaluate a pretrained Bidirectional GRU model on the copy task dataset.')
parser.add_argument('--model_path', type=str, default='trained_models/bi_rnn2',
...
# define model (using BidirectionalGRUDecoder instead of GRUDecoder)
...
# near top: build the device from CLI arg
...
# Explicitly allow a full unpickle (not weights-only) and remap to your device
...
output_file = os.path.join(model_path, f'bigru_rnn_{eval_type}_predicted_sentences_{time.strftime("%Y%m%d_%H%M%S")}.csv')
